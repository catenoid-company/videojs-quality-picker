...
function qualityPickerPlugin(options) {
    ...
    function onFragChanged(event, data) {
        if (typeof(player.controlBar.getChild(SUPPORTED_TRACKS[0] + 'PickerButton')) !== 'undefined') {
            ...
            if (data.frag.autoLevel) {
                ...
                auto_menu.el_.childNodes[0].data = 'auto (' + QUALITY_DATA.video[data.frag.level + 1].label + ')';
            } else {
